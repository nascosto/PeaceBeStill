...
| `channelTabs`             | a channel's Posts and Store tabs; a direct visit to one of those pages goes to the channel home | `yt-tab-shape:is([tab-title="Posts"], [tab-title="Store"])`; content.js `location.replace(channelHomeFor(pathname))` on load and navigation |
